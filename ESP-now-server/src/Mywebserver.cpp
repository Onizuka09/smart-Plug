...
void OnAllowed_devices(){
  server.on("/allowed_devcies", HTTP_POST, HandleAllowed_dev);
}
...
String MAc    = doc["MAC"].as<String>();
String uuid = doc["uuid"].as<String>();
int st = doc["function"].as<int>();
// if (st ==2 ){
 //  Serial.println("handling timer" );
  gtw_state = gateway_Handle_timer; 
// }
// else {
  String d ; 
  serializeJson(doc, d );
  Serial.println("error handling timer " );
  Serial.println(d);
// gtw_state = gateway_handle_off ; 
// }
 server.send(200, "text/plain", "data received correctly ");
